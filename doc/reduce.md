...
默认 `N = 1 << 22`、`blockDim.x = 256`、`kItemsPerThread = 16` 时：

| version | 每 block 处理元素 | 全局 `atomicAdd` 次数 |
| --- | ---: | ---: |
| v2_smem | 256 | 16,384 |
| v3_items16 | 4,096 | 1,024 |
...
全局 `atomicAdd` 次数从 `ceil(N / T)` 降为 `ceil(N / (T * K))`。默认配置下是从 `16,384` 次降到 `1,024` 次。
...
| version | ms | GB/s | TFLOPS | max_err |
| --- | ---: | ---: | ---: | ---: |
| v3_items16 | 0.4043 | 41.50 | 0.0104 | 0.000000 |
...
### 为什么可能更快
v3 还有 `ceil(N / (blockDim.x * kItemsPerThread))` 次全局 `atomicAdd`，默认是 `1,024` 次，而且这些原子加法都争抢同一个 `y` 地址。

v4 第一轮每个 block 写不同的 `partial[blockIdx.x]`，没有同地址竞争；第二轮只用一个 block 归约 1,024 个 partial，最后普通写一次 `y[0]`。因此它主要去掉了跨 block 合并阶段的 atomic contention。
...
默认 `N = 1 << 22`、`T = 256`、`K = 16` 时，`P = 1,024`。额外 partial 读写约 `1,024 * 4 * 2 = 8 KB`，相对输入的 `16 MB` 很小。
...
| version | ms | GB/s | TFLOPS | max_err |
| --- | ---: | ---: | ---: | ---: |
| v4_two_pass | 0.3551 | 47.27 | 0.0118 | 0.000122 |

对比当前 `v3_items16 = 0.4043 ms`，v4 约快 `1.14x`。这里的 `GB/s` 对 v4 计入了 `partial` 的一次写和一次读，因此和 v3 的有效字节口径略有差异，但额外字节很小，不影响主要结论。默认 benchmark 现在使用 `warmup=3, iters=20`，因此小幅优化结论需要复测确认。
...
| --- | --- | ---: | --- | --- |
| naive | 每个线程直接 `atomicAdd(y, x[idx])` | `N` | 同地址全局原子竞争 | 已测：`8.2726 ms`，`2.03 GB/s` |
| v2_smem | block 内 shared memory 树形归约，每个 block 一次全局 `atomicAdd` | `ceil(N / blockDim.x)` | block 内同步 + 剩余全局原子竞争 | 已测：`1.4927 ms`，`11.24 GB/s` |
| v3_items16 | 每线程 register 预累加 16 个元素，再做 block reduce | `ceil(N / (blockDim.x * 16))` | 输入读取 + block 内同步 + 剩余全局原子竞争 | 已测：`0.4043 ms`，`41.50 GB/s` |
| v4_two_pass | 每个 block 写 partial，第二轮归约 partial | 0 | 输入读取 + block 内同步 + kernel launch | 已测：`0.3551 ms`，`47.27 GB/s` |
