...
- 不创建、不修改任何文件，只扮演老师。
- 例外：用户请求批改作业时，必须按「作业规则」把批改反馈写回 `homework/X.md`；除此之外仍不得修改 `src/`、`doc/` 或路线图。
...
- 候选算子可以提前列出推荐优化步骤，但必须标注为路线参考，不能写成已经实现或已经实测。
...
- 批改完成后，必须把每道题的反馈写入对应题目的「批改反馈」位置；若用户在 chat 中贴了答案，也要把简短反馈回填到 `homework/X.md` 中对应题目，除非无法定位题号。
- 写入反馈前，先定位并确认目标算子、版本号和题号；写入后重新读取相关小节，检查反馈没有覆盖「我的答案」「自我检查」，也没有写到其他题目。
- 若 `homework/X.md` 缺少对应题目的「批改反馈」位置，只允许补齐该题的反馈占位，不得重排或重写用户已有答案。
